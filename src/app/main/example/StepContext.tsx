...
export declare type ValidationSchema = Record<
  string,
  {
    ...
    error?: string;
    required?: boolean;
    validate?: 'text' | 'number' | 'email' | 'phone' | 'zip' | 'checkbox' | 'select' | 'age';
    minLength?: number;
    maxLength?: number;
    helperText?: string;
  }
...
export function StepsProvider({ children }: ProviderProps) {
      ...
      if (validate) {
        switch (validate) {
          ...
          case 'phone':
            ...
            break;
          case 'age':
            if (value && !isAge.test(value))
              error = helperText || 'Please enter a valid age number. i.e: xxx';
            break;
  ...
  return (
    <AppContext.Provider value={constextValue}>
      <div className="mui-step-form w-full flex">{children}</div>
    </AppContext.Provider>
  );
}
